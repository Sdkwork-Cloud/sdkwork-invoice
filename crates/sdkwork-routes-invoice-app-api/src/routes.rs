...
use crate::{app_invoice_router_with_postgres_pool, app_invoice_router_with_sqlite_pool};

pub fn build_invoice_app_router(host: Arc<InvoiceServiceHost>) -> Router {
    match host.database_pool() {
        DatabasePool::Postgres(pool, _) => app_invoice_router_with_postgres_pool(pool.clone()),
